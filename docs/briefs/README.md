...
| 28 | [KICKTIPP_QUELLENLABEL_PROMPT.md](KICKTIPP_QUELLENLABEL_PROMPT.md) | App B: „entrandet" → „ohne Marge" + Erklärsatz (Jargon-Scan); Quotenquelle über die Marge benennen (`oddsSourceLabel`, Schwelle 2 %); Rechenweg-Darstellung nach `render.mjs` extrahiert + jsdom-Wächter, dass die Modell-Zeile den `odds:null`-Lauf zeigt. Keine Optimierungs-/Scoring-Änderung, kein Release. |
| 29 | [PREMATCH_FENSTER_BRIEF.md](PREMATCH_FENSTER_BRIEF.md) | Pipeline-Defektfix: Pre-Match-Einträge werden bis zum Anstoß **neu berechnet** und erst **ab Anstoß** eingefroren (write-once wurde zu früh angewandt — 612 Einträge der Saison hingen an einem Julisnapshot). Substanz-Regel gegen Commit-Rauschen, Anstoßgrenze am Laufzeitpunkt. Die verworfene Fenster-Variante ist im Briefkopf dokumentiert. Release 2.3.3. |
| 30 | [NACHHOLSPIEL_FREEZE_BRIEF.md](NACHHOLSPIEL_FREEZE_BRIEF.md) | Codex-Audit zu 29: die Einfrierbedingung las auch den *gespeicherten* Anstoß und fror ein verlegtes Spiel auf dem alten Datum ein — es friert ab jetzt **das Ergebnis oder der aktuelle Anstoß**, `kickoff` kommt in die Substanz-Liste. Dazu Lockfile-Gleichlauf als stehende Regel plus Wächter (`npm ci` akzeptiert die Drift stillschweigend). Release 2.3.4. |
| 31 | [AUDIT_FAMILIE_BRIEF.md](AUDIT_FAMILIE_BRIEF.md) | Interner Sweep nach der Freeze-Familie: Timeline-Punkte cachten die Prämisse „abgeschlossener Spieltag" auf der falschen Bedingung — ein Spieltag mit verlegtem Spiel ging mit 8 von 9 Ergebnissen dauerhaft in den Cache, der Live-Stichtag hing vom Berechnungszeitpunkt ab, retro- und live-gebaute Timelines bekamen verschiedene Bedeutung. Fix: **kumulative Vollständigkeit** (Punkt M braucht alle Spiele 1–M). Dazu Grenzfall-Methode und Bestandsprüfung. Release 2.3.5. |
| 32 | [ERSCHEINUNGSBILD_MARKE_PROMPT.md](ERSCHEINUNGSBILD_MARKE_PROMPT.md) | Erscheinungsbild, reine Präsentation: das Balkenmotiv des OG-Bildes wird eine Bildfamilie — Marke links neben dem h1 über die geteilte `SiteBrand` (beide Header-Varianten, `aria-hidden`, Hex nur im SVG-Asset), `og-image.png` als README-Banner über den Badges (relativer Pfad), Favicon auf dasselbe Motiv umgestellt (§3, Nutzerentscheidung: ja). Kein Engine-/Pipeline-/Datenbezug. Release 2.3.6. |
| 33 | [HALBSERIEN_BRIEF.md](HALBSERIEN_BRIEF.md) | Halbserien-Paket: Herbstmeister-Tally in der Engine (liest gezogene Ergebnisse, zieht nichts — bestehende Artefaktzahlen über den `ENGINE_VERSION`-Bump bitidentisch), Anker in **beiden** Ligen (§7b), Halbserien-Tabellen/-Bilanzen/-Marker in der App, Halbzeitbilanz auf Modellgüte, Drei-Anker-Vergleich auf Verlauf. Historien-Artefakte einmalig regeneriert. Release 2.4.0. |
| 34 | [ENTKOPPLUNG_ERGEBNISSE_RATINGS_BRIEF.md](ENTKOPPLUNG_ERGEBNISSE_RATINGS_BRIEF.md) | Ergebnisse und Ratings sind zwei Kanäle: `--no-ratings-fetch` rechnet die Prognose aus dem neuesten **vollständigen** archivierten Snapshot und stempelt sie, `data.yml` fasst clubelo nicht mehr an (alle 15 min), `ratings.yml` archiviert stündlich auf eigenem `betrieb`-Kanal. Zwei Uhren in `meta.json`, Rating-Aktualität im Header (grün/orange). Weg A bleibt verworfen. Release 2.5.0. |
